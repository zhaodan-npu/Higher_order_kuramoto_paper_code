#!/usr/bin/env python3
"""
读取第一通量时间（FPT）结果文件 fpt_results.txt，
经过高斯平滑后生成出版级热力图，并保存为 PDF。
使用 cmcrameri 的 'oslo_r' 顺序色图。
"""
import numpy as np
...
# ---------- 2. 平滑处理 ----------
# 对 FPT 矩阵进行高斯平滑
SMOOTH_SIGMA_K1 = 1.0  # 在 K1 方向上的平滑强度
SMOOTH_SIGMA_K2 = 1.5  # 在 K2 方向上的平滑强度
...
# 添加等高线（可选）
# levels = np.linspace(fpt_mat.min(), fpt_mat.max(), 7)
# cs = ax.contour(
#     fpt_smooth, levels=levels,
#     extent=[K2_vals.min(), K2_vals.max(), K1_vals.min(), K1_vals.max()],
#     colors='white', linewidths=1.2
# )
# ax.clabel(cs, fmt="%.1f", fontsize=14, inline=True)
