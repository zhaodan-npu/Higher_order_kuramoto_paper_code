...
def main():
    ...
    np.savetxt("pdf_matrix.txt", pdf_matrix)

    plt.figure(figsize=(10, 8))
    plt.imshow(pdf_matrix, extent=[x_vals.min(), x_vals.max(), alpha_vals.min(), alpha_vals.max()],
               aspect='auto', origin='lower', cmap='viridis')
    plt.colorbar(label='PDF')
    plt.xlabel("Order Parameter r")
    plt.ylabel("Alpha")
    plt.title("PDF of Order Parameter r for Different Alpha Values (sigma=0.1)")
    plt.savefig("pdf_heatmap.png")
    plt.show()
